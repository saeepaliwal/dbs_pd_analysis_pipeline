function test_tapas_mpdcm_fmri_int_host(fp)
%% Test 
%
...
test_tapas_mpdcm_fmri_int_host_memory(fp, 'euler')
test_tapas_mpdcm_fmri_int_host_memory(fp, 'rk4')

test_tapas_mpdcm_fmri_int_host_correctness(fp, 'euler')
test_tapas_mpdcm_fmri_int_host_correctness(fp, 'rk4')

end

function test_tapas_mpdcm_fmri_int_host_memory(fp, integ)
...
function test_tapas_mpdcm_fmri_int_host_correctness(fp, integ)
...
for i = 1:5
    ...
    theta{1}.B = dcm.B;
    theta{1}.C = dcm.C;
    ny = tapas_mpdcm_fmri_int_host(u, theta, ptheta);
    if all(abs(dcm.y - ny{1}) < tol)
        ...
        fprintf(fp, 'Accuracy not bellow the tolerance %0.5f\n', tol)
    end
...
function [ dcm ] = cached_values(d)
%% Caches values for the test of the accuracy

f = mfilename('fullpath');

[tdir, ~, ~] = fileparts(f);

fname = fullfile(tdir, 'test_accuracy.mat');
...
y = 1;
try
    y = tapas_mpdcm_fmri_int_host(u, theta, ptheta);
    fprintf(fp, '    Passed\n')
catch err
    d = dbstack();
    fprintf(fp, '   Not passed at line %d\n', d(1).line)
