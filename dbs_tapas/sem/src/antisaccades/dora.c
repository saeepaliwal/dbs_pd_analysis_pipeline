...
<<<<<<< HEAD
        ...
        fllh = wald_lpdf(t, kp, tp);
        fllh += log(wald_sf(t, ks, ts));

        if ( t > da )
        {
            fllh += log(wald_sf(t - da, ka, ta));
            fllh += log(wald_sf(t - da, kl, tl));
        }
        ...
        } else 
        {
            ...
            sllh = wald_lpdf(t - da, kl, tl) 
                + log(wald_sf(t - da, ka, ta))
                + log(cumint + wald_sf(t, kp, tp)); 
        }
    
        fllh = log(0.999 * exp(fllh) + exp(sllh)) + log(1 - p0);
        break;
    ...
    case ANTISACCADE:
        ...
        } else 
        {
            ...
            sllh = wald_lpdf(t - da, ka, ta) + 
                log(wald_sf(t - da, kl, tl)) +
                log(cumint + wald_sf(t, kp, tp)); 
        }
        fllh = log(0.001 * exp(fllh) + exp(sllh)) + log(1 - p0);
        break;
	}
