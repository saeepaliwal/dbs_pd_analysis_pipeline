...
for p  = {'omega','theta','beta'}
    for i = 1:2
        ...
        fprintf('%s %s -- Mean: %0.3f (%0.3f, %0.3f-%0.3f)\n',timepoint,p{:},mean(stats{i}.(p{:})), ...
        std(stats{i}.(p{:})),min(stats{i}.(p{:})), max(stats{i}.(p{:})))
    end
end
...
for p  = {'omega','theta','beta'}
    ...
    [h, pval,c,info] = ttest(stats{1}.(p{:}),stats{2}.(p{:}));

    fprintf('Tstat %s: %0.3f p-val: %0.3f\n',p{:},info.tstat, pval)
...
if display_figure
    ...
    subplot(1,3,2);
    h = boxplot([stats{1}.theta,stats{2}.theta],'Symbol','k.');
...
ylabel('Free energy')
purty_plot(101,['model_comparison'],'png')
