function test_tapas_linear_hier_llh(fp)
%% Test 
%
...
try
    tapas_linear_hier_llh(state.graph{2}, state.graph{3}, ...
...
% Test whether there is any clear bug
try
    [llh] = tapas_linear_hier_llh(state.graph{2}, state.graph{3}, ...
        model.graph{2}.htheta);
...
% Test whether there is any clear bug
try
    tapas_linear_hier_llh(state.graph{3}, state.graph{4}, ...
...
% Test whether there is any clear bug
try
    [llh] = tapas_linear_hier_llh(state.graph{3}, state.graph{4}, ...
        model.graph{3}.htheta);
    assert(size(llh, 2) == numel(pars.T), 'llh shape is wrong');
    fprintf(fp, '       Passed\n');
catch err
