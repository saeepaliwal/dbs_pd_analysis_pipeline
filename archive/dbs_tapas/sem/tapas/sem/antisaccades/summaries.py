...
<<<<<<< HEAD
        time = np.linspace(0.0, 13.0, 300)

        results['e'] = stheta.tp / (stheta.kp - 1) + stheta.t0

        pr = stats.invgamma.pdf(time, stheta.ka, 
		scale=stheta.ta, loc=stheta.t0 + stheta.da) * \
            stats.invgamma.sf(time, stheta.kl, 
                scale=stheta.tl, loc=stheta.t0 + stheta.da) 
        
        results['pip'] = np.trapz(pr, time)
        results['a'] = np.trapz(time * pr, time) / results['pip']

        results['p'] = np.trapz(time * pr, time) / (1.0 - results['pia'])
        results['s'] = stheta.ts / (stheta.ks - 1.0) + stheta.t0

        results['pip'] = results['pip'] / (results['pip'] + \
                results['pia'])
...
<<<<<<< HEAD
        ...
        pr = stats.gamma.logpdf(time, stheta.ka, 
		scale=stheta.ta, loc=stheta.t0 + stheta.da) + \
        ...
        pr = stats.gamma.logpdf(time, stheta.kl, 
                scale=stheta.tl, loc=stheta.t0 + stheta.da) + \
        ...
        pr = stats.invgamma.pdf(time, stheta.kp, scale=stheta.tp) * \
                stats.invgamma.sf(time, stheta.ks, scale=stheta.ts) * \
                stats.gamma.sf(time, stheta.ka, scale=stheta.ta, 
                        loc=stheta.da) * \
                stats.gamma.sf(time, stheta.kl, scale=stheta.tl,
                        loc=stheta.da) * (1 - stheta.p0)
        #_()
        results['pip'] = np.trapz(pr, time)
